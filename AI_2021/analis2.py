...
if graph:
    ...
    ax1.plot(x, actions, color='b')
    ax.set_ylabel('Z')
    ax1.set_ylabel('action')
    ax.set_title("Текущие действия ")
    plt.show()


    fig, ax = plt.subplots(1,2)
    #ax[0].plot(x, alg_1_z_body, color='orange', label="alg_1_z_body")
    ax[0].plot(x, tr_z_body, color='orange', label="nn_z_body")
    ax[0].plot(x, body_z, color='red', label="body_z")
    ax[1].plot(x, alg_1_speed_treadmill, color='orange', label="alg_1_speed_treadmill")
